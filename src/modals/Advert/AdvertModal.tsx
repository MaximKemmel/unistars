...
export const AdvertModal: React.FC<IAdvertModalProps> = ({ isShow, onSave, onClose }) => {
        ...
        <form onSubmit={handleOnSubmit} className={modalStyles.form}>
          <div className={modalStyles.form_content} id="form">
            <div className={modalStyles.part_container}>
              <div className={`${modalStyles.part_multi} ${modalStyles.double}`}>
                <div className={modalStyles.part}>
                  ...
                  <input
                    ...
                    maxLength={16}
                  />
                  <div className={modalStyles.input_length}>{`${currentAdvert.title.length}/16`}</div>
                </div>
                <div className={modalStyles.part}>
                  ...
                  <input
                    ...
                    maxLength={56}
                  />
                  <div className={modalStyles.input_length}>{`${currentAdvert.subtitle.length}/56`}</div>
                </div>
              </div>
              <div className={`${modalStyles.part_multi} ${modalStyles.double}`}>
                <div className={modalStyles.part}>
                  ...
                  <div className={styles.cover}>
                    ...
                    {currentAdvert.imageUrl.trim().length === 0 ? (
                      <div className={styles.form_button_label}>{t("advertisements.choose_cover")}</div>
                    ) : (
                      <div className={styles.file_info}>
                        <div className={styles.file_name}>
                          <img src={FileIcon} alt="" />
                          <div className={styles.name}>{currentAdvert.imageUrl}</div>
                        </div>
                        {isImageUploaded ? (
                          <>
                ...
                <div className={modalStyles.part}>
                  <div className={modalStyles.part_label}>{`${t("advertisements.link_to_the_banner")} *`}</div>
                  <input
                    ...
                    onChange={(event) =>
                      setCurrentAdvert({
                        ...currentAdvert,
                        websiteUrl: event.target.value.trim(),
                      })
                    }
              ...
              <div className={`${modalStyles.part_multi} ${modalStyles.double}`}>
                <div className={modalStyles.part}>
                  <div className={modalStyles.part_label}>{t("advertisements.duration")}</div>
                ...
                <div className={modalStyles.part}>
                  ...
                  <input placeholder={"example@mail.ru"} type="text" required value={""} />
                  <div className={modalStyles.input_description}>{t("advertisements.manager_will_write")}</div>
                </div>
              </div>
            </div>
          </div>
